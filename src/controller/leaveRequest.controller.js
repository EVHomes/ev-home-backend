...
export const getLeave = async (req,res,next) =>{
  const{ applicant , reportingTo , leaveStatus }= req.query;
    try{
      ...
      if(leaveStatus){
        ...
        const resp = await leaveRequestModel.find(query)
        ...
        if (resp.length === 0) {
          ...
          return res.status(200).send(successRes(200, "Leave records retrieved", { data: resp }));
        } catch (error) {
  ...
  export const addLeave = async (req, res, next) => {
    ...
    const { leaveType,appliedOn,startDate,endDate,numberOfDays, reason,approveReason,leaveStatus, reportingTo, applicant,attachedFile } = req.body;
  
    try {
    
      if (!startDate) return res.status(401).send(errorRes(401, "Start Date is required"));
      if (!endDate) return res.status(401).send(errorRes(401, "End Date is required"));
